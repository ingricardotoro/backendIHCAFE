...
export async function findAtlasAccountsByProjAct(req, res) {
  ...
  try {
    ...
    const results = await sequelizeDb.query(
      "SELECT DISTINCT(budgetlines_atlas.code_atlas) AS code_atlas2,atlas_account.name AS name,  atlas_account.code AS code FROM budgetlines_atlas , atlas_accounts  Where budgetlines_atlas.code_atlas = atlas_account.id  AND budgetlines_atlas.project_id = " + project_id + " AND budgetlines_atlas.status = 'Aprobado' AND budgetlines_atlas.code_activity = '" + code_activity + "'", { type: sequelize.QueryTypes.SELECT }
    )
